...
describe('ResetPasswordPage', () => {
  ...
  it('muestra alerta de error cuando no hay token en la URL', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(/enlace de recuperación es inválido/i);
  ...
  it('muestra error de validación cuando las contraseñas no coinciden', async () => {
    ...
    await user.type(screen.getByLabelText('Confirmar contraseña'), 'OtraPassword1');
    await user.click(screen.getByRole('button', { name: /restablecer contraseña/i }));
  ...
  it('llama a resetPassword con el token de la URL y la nueva contraseña', async () => {
    ...
    await user.type(screen.getByLabelText('Confirmar contraseña'), 'NuevaClave1');
    await user.click(screen.getByRole('button', { name: /restablecer contraseña/i }));
  ...
  it('muestra alerta de error cuando el token es inválido o expirado', async () => {
    const user = userEvent.setup();
    mockResetPassword.mockRejectedValue(new Error('El enlace expiró o ya fue utilizado.'));

...
describe('RegisterPage', () => {
  ...
  it('muestra error de validación cuando la contraseña es débil', async () => {
    ...
    await user.type(screen.getByLabelText('Correo electrónico'), 'ana@nn.com');
  ...
  it('muestra mensaje de verificación de email tras registro exitoso', async () => {
    ...
    expect(screen.getByText(/te hemos enviado un correo de verificación/i)).toBeInTheDocument();
  ...
  it('muestra el error del servidor cuando el email ya está registrado', async () => {
    ...
    await user.type(screen.getByLabelText('Nombre completo'), 'Ana García');
    await user.type(screen.getByLabelText('Correo electrónico'), 'existente@nn.com');
    await user.type(screen.getByLabelText('Contraseña'), 'Password1');
    await user.type(screen.getByLabelText('Confirmar contraseña'), 'Password1');
    await user.click(screen.getByRole('button', { name: /crear cuenta/i }));

...
describe('VerifyEmailPage', () => {
  ...
  it('muestra alerta de error cuando no hay token en la URL', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent(/no se encontró el token de verificación/i);
  ...
  it('llama a la API con el token de la URL al montar el componente', async () => {
    ...
    await waitFor(() => expect(mockVerifyEmail).toHaveBeenCalledWith({ token: 'abc-token-123' }));
  });
  ...
  it('muestra estado de éxito cuando la API responde correctamente', async () => {
    ...
    expect(screen.getByText(/tu correo ha sido verificado/i)).toBeInTheDocument();
  });
  ...
  it('muestra el error de la API cuando el token es inválido', async () => {
    ...
    expect(await screen.findByText(/el token de verificación es inválido/i)).toBeInTheDocument();
  });

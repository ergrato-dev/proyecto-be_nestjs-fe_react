...
describe('ForgotPasswordPage', () => {
  ...
  it(
    ...
    async () => {
      ...
      await user.type(screen.getByLabelText('Correo electrónico'), 'registrado@nn.com');
      await user.click(screen.getByRole('button', { name: /enviar enlace/i }));

      // ¿Qué? El mensaje es genérico — no diferencia entre email registrado y no registrado.
      expect(await screen.findByRole('alert')).toHaveTextContent(/si tu correo está registrado/i);
    },
  );
  ...
  it(
    ...
    async () => {
      ...
      await user.type(screen.getByLabelText('Correo electrónico'), 'noexiste@nn.com');
      await user.click(screen.getByRole('button', { name: /enviar enlace/i }));

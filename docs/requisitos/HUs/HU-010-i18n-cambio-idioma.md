...
| RN-065 | El campo `locale` en la entidad `User` (columna `locale`) tiene valor `"es"` por defecto. |
| RN-066 | El endpoint `PATCH /me/locale` requiere autenticación con access token válido (`JwtAuthGuard`). |
...
- El hook `useTheme` gestiona el modo visual (dark/light). La lógica en `i18n/index.ts` gestiona el idioma.
- La librería de i18n utilizada en este proyecto es `i18next` con `react-i18next` (idéntica a la de los repos hermanos — el cambio de stack es solo en el backend).
- El idioma se aplica globalmente mediante el proveedor `I18nextProvider` en `main.tsx`.
- El DTO `UpdateLocaleDto` valida con `class-validator` (`@IsIn(['es', 'en'])`) que el body solo acepte los locales soportados.

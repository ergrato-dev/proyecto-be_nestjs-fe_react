...
- `AuthService.changePassword()` verifica `currentPassword` con bcrypt antes de aceptar el cambio.
- La ruta está protegida con `@UseGuards(JwtAuthGuard)` en `AuthController`.

...
> **Concepto pedagógico:** La diferencia entre un sistema stateless (JWT, como este) y stateful (sesiones de servidor) es precisamente ésta. Con `@nestjs/jwt` + Passport, el servidor no sabe quién está "conectado" — la invalidación de sesión requiere mecanismos adicionales como blacklists.

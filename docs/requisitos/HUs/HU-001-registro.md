...
- El campo `isEmailVerified` (columna `is_email_verified`) se establece en `false` al registrarse; el login retorna `403` hasta que sea `true`.
- El token de verificación de email se genera en `AuthService`, se persiste en la entidad `EmailVerificationToken` (tabla `email_verification_tokens`) y expira en 24 horas.

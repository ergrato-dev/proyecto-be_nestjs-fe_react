...
- **entonces** se retornan un nuevo `accessToken` Y un nuevo `refreshToken` (rotación), y el anterior queda obsoleto.
...
- La rotación de tokens implica que `AuthService.refresh()` firma un nuevo `refreshToken` en cada llamada — esto evita que un refresh token comprometido pueda usarse indefinidamente.
- El claim `type` se verifica en el payload del JWT — un access token no puede usarse como refresh token. `@nestjs/jwt` firma ambos tokens con secretos distintos (`JWT_ACCESS_SECRET` / `JWT_REFRESH_SECRET`).

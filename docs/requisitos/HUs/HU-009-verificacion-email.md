...
- **entonces** mi cuenta queda activa (`isEmailVerified = true`), el token se marca como usado, y soy redirigido al login con el mensaje: "¡Email verificado correctamente! Ya puedes iniciar sesión".
...
- **Dado que** mi cuenta existe pero `isEmailVerified = false`,
...
| ID     | Regla                                                                                                   |
| ------ | ---------------------------------------------------------------------------------------------------------- |
| RN-050 | El token de verificación se genera con el módulo `crypto` de Node (`randomBytes(32)`) — 256 bits de entropía. |
...
| RN-053 | El servidor siempre retorna HTTP 201 al registrarse, independientemente de si el email ya existe (anti-enumeración parcial). El bloqueo de emails duplicados ocurre con HTTP 409, pero el mensaje es genérico. |
| RN-054 | El campo `isEmailVerified` en la entidad `User` tiene valor `false` por defecto.                       |
...
- La entidad `EmailVerificationToken` (tabla `email_verification_tokens`) almacena los tokens con relación `userId → users(id)`, declarada con `@ManyToOne()` en TypeORM.

...
- **entonces** el campo `hashedPassword` no aparece en ningún momento.
...
- El `userId` del endpoint lo provee exclusivamente el `JwtAuthGuard` + `JwtStrategy` (nunca el body ni parámetros de ruta) — previene IDOR (OWASP A01).
- La respuesta incluye `isEmailVerified` y `locale` para que el frontend pueda mostrar avisos y aplicar el idioma guardado.

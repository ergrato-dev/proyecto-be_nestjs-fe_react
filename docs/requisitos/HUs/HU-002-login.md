...
- El campo `isEmailVerified` es verificado en `AuthService.login()` como condición previa — un `false` retorna HTTP 403 (`ForbiddenException`).
- El campo `isActive` es verificado como condición previa al login — una cuenta desactivada retorna `403`.
- El rate limiting se implementa con `@nestjs/throttler` (`ThrottlerGuard`) aplicado sobre el `AuthController`.

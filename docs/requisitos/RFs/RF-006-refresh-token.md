...
usando `@nestjs/jwt` para firmar y verificar.
...
| 2    | El `ValidationPipe` valida el body contra `RefreshTokenDto`.                                     |
| 3    | `AuthService.refresh()` verifica la firma JWT del `refreshToken` con `JWT_REFRESH_SECRET` (vía `JwtService.verifyAsync()`). |
| 4    | Si el token es inválido o expirado, se lanza `UnauthorizedException`.                            |
| 5    | Se verifica en el payload que `type === "refresh"` — evita usar un access token como refresh.   |
| 6    | Se busca al usuario por `userId` del payload vía `Repository<User>` — si no existe, `UnauthorizedException`. |
...
**Input** (body JSON, validado por `RefreshTokenDto`):
```json
{ "refreshToken": "eyJ..." }
```
...
```json
{ "statusCode": 401, "message": "Invalid or expired refresh token", "error": "Unauthorized" }
```

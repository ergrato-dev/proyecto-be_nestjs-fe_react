# Setup con Docker — NN Auth System (NestJS Edition)
...
<!--
  ...
  ¿Impacto? Es el modo recomendado para desarrollo: Node.js y pnpm corren nativamente
             (hot-reload inmediato vía Nest CLI watch), y los servicios de infraestructura
             viven en Docker.
-->
...
| Backend    | `pnpm start:dev` (nativo) | http://localhost:3000 |
...
```bash
# Desde be/ — con Docker corriendo. Compila primero (el TypeORM CLI lee dist/)
pnpm build
...
pnpm test:e2e        # tests end-to-end (Jest + supertest)
pnpm lint            # verificar errores de ESLint
pnpm format          # formatear con Prettier
pnpm build           # compilar TypeScript → dist/

# ─── TypeORM ───
pnpm typeorm migration:generate src/migrations/Nombre  # generar migración desde las entidades
pnpm typeorm migration:run                              # aplicar migraciones pendientes
pnpm typeorm migration:revert                           # revertir la última migración
pnpm typeorm migration:show                              # ver estado de las migraciones
...
# Tests unitarios (requiere Docker corriendo — PostgreSQL activo para los que integran BD)
pnpm test

# Con cobertura
pnpm test:cov

# Tests end-to-end (requieren PostgreSQL activo)
pnpm test:e2e
...
# (en la terminal donde corre pnpm start:dev)
```
...
docker compose up -d
cd be && pnpm build && pnpm typeorm migration:run
```

# Setup sin Docker — NN Auth System (NestJS Edition)
...
| Backend    | `pnpm start:dev` (Nest CLI watch)  | http://localhost:3000 |
...
```bash
# Desde be/ — el TypeORM CLI lee las migraciones compiladas en dist/
pnpm build
...
```
[✓] Migration InitialSchema1745000000000 has been executed successfully.
```
...
**consola** donde corre `pnpm start:dev`. Copia el enlace desde la terminal para usarlo.
...
### Terminal 1 — Backend (NestJS + Nest CLI watch)
...
# → {"id":"...","email":"test@example.com",...,"isEmailVerified":false}
...
curl -X POST http://localhost:3000/api/v1/auth/verify-email \
  -H "Content-Type: application/json" \
  -d '{"token":"<token-copiado-del-email-o-de-los-logs>"}'

# 4. Iniciar sesión (solo funciona tras verificar el email)
...
```bash
# ─── Backend (desde be/) ───
pnpm start:dev       # servidor con hot-reload (Nest CLI watch)
pnpm test            # tests unitarios (Jest, requiere PostgreSQL para los que integran BD)
pnpm test:watch      # tests en modo watch
pnpm test:cov        # tests unitarios con cobertura
pnpm test:e2e        # tests end-to-end (Jest + supertest, requiere PostgreSQL activo)
pnpm lint            # verificar errores de ESLint
pnpm format          # formatear con Prettier
pnpm build           # compilar TypeScript → dist/

# ─── TypeORM (desde be/) ───
pnpm typeorm migration:generate src/migrations/Nombre  # generar migración desde entidades
pnpm typeorm migration:run                              # aplicar migraciones pendientes
pnpm typeorm migration:revert                           # revertir la última migración
pnpm typeorm migration:show                              # ver estado de las migraciones aplicadas
...
# Tests unitarios (algunos usan un Repository mockeado, no requieren BD)
pnpm test

# Con cobertura
pnpm test:cov

# Tests end-to-end — PostgreSQL debe estar activo
pnpm test:e2e
...
> ⚠️ Los tests e2e del backend usan la misma base de datos PostgreSQL con limpieza
> automática en cada test (`beforeEach` borra los datos). Asegúrate de que
> PostgreSQL esté corriendo antes de ejecutar `pnpm test:e2e`.
...
```bash
# Ver estado actual
cd be && pnpm typeorm migration:show
...
pnpm build && pnpm typeorm migration:run
```
...
git clone <url> && cd proyecto-be_nestjs-fe_react
...
# Backend
cd be && pnpm install && cp .env.example .env && pnpm build && pnpm typeorm migration:run

# Frontend
cd ../fe && pnpm install && cp .env.example .env

# Levantar (terminales separadas)
cd be && pnpm start:dev    # Terminal 1 → http://localhost:3000

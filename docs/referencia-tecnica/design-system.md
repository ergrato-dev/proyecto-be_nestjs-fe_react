...
### ¿Por qué `brand-*` y no `rose-*`?

Cada proyecto del sistema educativo tiene un color de acento distinto para que el alumno
identifique visualmente en qué stack está trabajando. Hardcodear `rose-*` en los componentes
...
| ---------------------- | -------------------------------- | -------------- | ---------------------- |
| **Express.js**         | `proyecto-be_express-fe_react`   | `blue`         | Azul clásico Node.js  |
...
| **NestJS**             | `proyecto-be_nestjs-fe_react`    | `rose`         | Rosa del logo NestJS — **este proyecto** |

### Shades utilizados

| Variable CSS          | Tailwind equivalente | Uso típico                                 |
| --------------------- | -------------------- | ------------------------------------------ |
| `--color-brand-400`   | `rose-400` etc.      | Dark mode: íconos, links, focus rings      |
| `--color-brand-500`   | `rose-500` etc.      | Borde del logo SVG, color de íconos neutro |
| `--color-brand-600`   | `rose-600` etc.      | Botones primarios en light mode            |
| `--color-brand-800`   | `rose-800` etc.      | Fondo de badges de color en light mode     |
...
```tsx
// Cambiar "NestJS" por el nombre del nuevo stack en el título del hero
<h1>
  {t('landing.title')}{' '}
  <span className="text-brand-600 dark:text-brand-500">NestJS</span>  {/* ← cambiar */}
</h1>
```
...
const techStack = [
  'Node.js 20',
  'NestJS 11',      // ← cambiar por el backend del nuevo stack
  'TypeORM',
  'TypeScript',
  // ...
] as const;
...
las herramientas correctas del nuevo stack (e.g. `class-validator` en vez de Zod, TypeORM
en vez de Drizzle).

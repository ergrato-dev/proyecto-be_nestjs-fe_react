...
- [ ] Elige **un** endpoint y escribe abajo, con tus propias palabras (10-15 líneas), el flujo
      completo request → response (Controller → Service → Repository TypeORM → DB → respuesta).
...
`be/src/auth/auth.controller.ts` → `auth.service.ts` →
`be/src/auth/strategies/jwt.strategy.ts` → TypeORM Repository → DB.

- [ ] ¿Dónde se genera el JWT (`@nestjs/jwt`)? ¿Dónde se valida en requests posteriores
      (`be/src/auth/guards/jwt-auth.guard.ts` + `jwt.strategy.ts`)?

```
(tu respuesta aquí)
```

- [ ] ¿Dónde se hashea el password (`bcryptjs`) y por qué ese algoritmo y no otro?
...
- Agregar un campo nuevo a la entidad `User` (`be/src/users/entities/user.entity.ts` + migración
  TypeORM + exponerlo en el frontend).
- Cambiar y justificar la expiración del access/refresh token.
- Agregar una regla de validación nueva con `class-validator` en el DTO de registro.
...
Extiende `fe/src/pages/DashboardPage.tsx` (o el módulo que lo respalde) con datos reales
obtenidos de un endpoint propio (no mock, no hardcode).
...
- [ ] ¿Tu endpoint/cambio nuevo requiere autenticación (`@UseGuards(JwtAuthGuard)`)? ¿Debería?
- [ ] ¿Valida el input recibido con un DTO + `class-validator`? ¿Qué pasa si mandas un valor
      inválido o vacío?
